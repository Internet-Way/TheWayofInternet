...
import {
  ...
  iconLinks,
  unicodeEmojiPlugin,
} from "./plugins/markdown/icon-transformer";
...
try {
  commitTitle = execSync("git log -1 --pretty=%h", {
    ...
    .toString()
    .trim();
  commitIso = execSync("git log -1 --format=%cI", {
    stdio: ["pipe", "pipe", "ignore"],
  })
    .toString()
    .trim();
...
export default defineConfig({
  ...
  head: [
    ...
    [
      "meta",
      {
        name: "description",
        content:
          "bitindex is a free media, resource, and tool index featuring alternatives, curated lists, and a digital vault.",
      },
    ],
    ["meta", { property: "og:type", content: "website" }],
    ["meta", { property: "og:title", content: "bitindex" }],
    [
      "meta",
      {
  ...
  themeConfig: {
    ...
    footer: {
      message: `Last Edited <time id="last-edited" datetime="${commitIso}">a few seconds</time> ago | <a href="https://github.com/Internet-Way/bitindex/commit/${commitTitle}">${commitTitle}</a><br/>Made with 🖤 | This site doesnt host any files`,
      copyright: "",
    },
  },
});
