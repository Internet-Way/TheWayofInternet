...
- **Every page carries `id`** — a URL-slug string (e.g. `id: privacy`, `id: get-started`, `id: notice`), used for stable page identity.
...
- `stats.data.ts` — `createContentLoader('**/*.md', { includeSrc })` → `PageStats` per page: `{ url, title, type, lines, departments[] }`. Parser skips YAML frontmatter, fenced blocks (```/~~~), HTML comments, and `***`/`---` rules; each scope stores its heading + anchor `slug` + direct lines/list items.
